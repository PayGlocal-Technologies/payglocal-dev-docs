...
## 0. Security: never read secrets (applies to AI agents)

**AI agents working in this repo must never read, echo, or inspect the contents of:**
- `.env`, `.env.*`, `*.env` files
...
- For "is this set?" checks, give the user a masked command to run themselves (e.g. `env | grep ^JIRA_ | sed 's/=.*/=****/'`).
- This rule takes priority over auto-mode autonomy. Pause if necessary.

See also `~/.claude/CLAUDE.md` (user-global version) and `./CLAUDE.md` (project version).
