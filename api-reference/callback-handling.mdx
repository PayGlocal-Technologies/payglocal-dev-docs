...
<Warning>Never process a callback payload before verifying its RS256 signature. An unverified x-gl-token could be forged. Verify first, then decode.</Warning>
...
// Step 1: Verify RS256 signature using PayGlocal's public certificate (PUBCERT)
// Download PUBCERT from GCC dashboard — see /set-up/keys/public-certificate
...
// Step 2: Token verified — safe to use payload
```

<Note>Return HTTP 200 to PayGlocal as soon as verification and acknowledgement is complete — even if your business processing is async. If you never receive the callback, fall back to [Get Transaction Status](/api-reference/payments-v2/common/get-transaction-status).</Note>
