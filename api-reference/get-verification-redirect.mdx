...
description: "Returns a time-limited iFrame URL for merchant document and video KYC/KYB verification after all prior onboarding steps are complete."
openapi: "GET /gcc/v2/partner/merchant/verification/{onboardingId}/redirect"
---
...
- `callBackUrl` is persisted on the merchant record and used as the post-verification redirect target. Its domain must be [whitelisted](/set-up/partner-onboarding/iframe-integration#step-1-domain-whitelisting-action-required).
...
```json Request
{
  "callBackUrl": "https://your-platform.example.com/onboarding/callback"
}
```
...
```json Response (200)
{
  "status": "SUCCESS",
  "data": {
    "redirectLink": "https://uat.dashboard.payglocal.in/app/partner-onboarding?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.example&onboardingId=<ONBOARDING_ID>"
  }
}
```
