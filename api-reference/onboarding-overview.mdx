...
<Note>
  This API is for **Partners** onboarding their own sub-merchants onto PayGlocal. If you are a merchant looking to accept payments, see the [Merchant Payment APIs](/api-reference/payment/overview) instead.
</Note>
...
<Note>
  The interactive API explorer on each endpoint page lets you test requests directly from this portal. Set your `x-api-key` and `x-gl-digest` headers in the authentication panel (top-right of the endpoint page) to make live Sandbox calls.
</Note>
...
| 8 | [Trigger Verification](/api-reference/verify-merchant) | PUT | Returns `redirectUrl` for iFrame VKYC |
...
- Calling `/verify` before all required steps are complete returns `400 Bad Request`.
