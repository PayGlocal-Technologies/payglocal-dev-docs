...
| Payment APIs | RSA-signed JWS in `x-gl-token-external` | `x-gl-token-external` (see [Key Management](/set-up/keys/overview)) |

You use your **PVT-KEY** to sign requests; **PUBCERT** verifies responses and may encrypt the body when your integration requires JWE. See [Key Management](/set-up/keys/overview) and [Constructing API Requests](/set-up/keys/request-construction) for setup.
...
Hosted **PayCollect** supports **UPI** and **UPI Intent** for INR. Product overview, screenshots, and a funds-flow diagram live on **[UPI](/payments/types/upi-overview)**. Request and response shapes (minimal vs enriched body, initiation vs `statusData` after success, headers, callbacks) are documented on **[UPI Intent](/api-reference/payment/upi-intent)**.

<CardGroup cols={2}>
  <Card title="UPI" icon="wallet" href="/payments/types/upi-overview">
...
<CardGroup cols={2}>
  <Card title="Which integration is right for you?" icon="map" href="/get-started/choose-your-path">
    PayCollect vs PayDirect vs Standalone vs Standing Instructions.
  </Card>
  <Card title="Payment Flow Overview" icon="diagram-project" href="/payments/lifecycle/initiation-flow">
