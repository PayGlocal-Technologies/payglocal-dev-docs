---
title: "UPI Intent"
description: "Initiate a UPI Intent payment via PayCollect: POST to /gl/v1/payments/initiate/paycollect, redirect the customer to the returned redirectUrl, and poll status for the final outcome."
openapi: "POST /gl/v1/payments/initiate/paycollect"
---

## When to Use

Use **UPI Intent** when your merchant uses hosted PayCollect for **INR UPI** and wants PayGlocal to collect the approval flow on its hosted checkout. Your backend calls **[GPI (Checkout Flow)](/api-reference/payments-v2/paycollect/gpi)** (`POST /gl/v1/payments/initiate/paycollect`); the JSON body is the **payment intent**. PayGlocal returns a **hosted `redirectUrl`** so the shopper completes **UPI_INTENT** via QR or in-flow approval.
...
Hosted PayCollect is the standard entry point for **UPI** in India: your server calls this endpoint with amount and callback URL, then sends the customer to `redirectUrl`. For the customer journey and funds flow see [UPI](/payments/types/upi-overview). This page covers UPI Intent request/response details, including optional `billingData` / `riskData` and the post-success `statusData` payload.
...
<Note>
  Traffic captured from **`gcc.uat.payglocal.in`** (merchant console “Test transaction”) may use a different path and **browser cookies**. For production integrations, call only **`api.uat.payglocal.in`** or **`api.payglocal.in`** with [RSA-signed JWS](/set-up/keys/overview) in `x-gl-token-external`.
</Note>
...
- `merchantCallbackURL`: HTTPS endpoint PayGlocal POSTs to after checkout completes; verify the `x-gl-token` in the callback before trusting the payload (see [Payment Initiation Flow](/payments/lifecycle/initiation-flow)).
...
| `x-gl-token-external` | Yes | RSA-signed **JWS** for the Payment API per [Key Management](/set-up/keys/overview). |
...
1. **HTTP GET** redirect the shopper to `redirectUrl` from Response A ([Payment Initiation Flow](/payments/lifecycle/initiation-flow)).
...
```json Response (200)
{
  ...
  "data": {
    ...
    "data": {
      ...
      "statusUrl": "https://api.uat.payglocal.in/gl/v1/payments/gl_o-9f9ff90e82cfafb07515k0ZX2/status?x-gl-token=<TOKEN>",
      "merchantTxnId": "23AEE8CB6B62EE2AF07"
    },
    "errors": null
  },
...
| Scenario | HTTP Code |
|----------|-----------|
| Missing or invalid required fields (`merchantTxnId`, `totalAmount`, `txnCurrency`) | 400 |
| Invalid or expired `x-gl-token-external` JWS signature | 401 |
| Merchant ID not found or not authorised for PayCollect | 403 |
| Duplicate `merchantTxnId` for the same merchant | 409 |
| Customer abandons the hosted page — no callback received | — (poll Get Status) |
| Callback `x-gl-token` validation fails | — (use Get Status with stored `gid`) |
