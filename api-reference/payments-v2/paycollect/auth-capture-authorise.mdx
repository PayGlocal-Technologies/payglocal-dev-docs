---
title: "PayCollect — Authorize Payment (Auth Only)"
...
Authorize (auth only) on hosted checkout supports **cards** and **international Apple Pay** only. **UPI and net banking are not supported** for auth transactions (they are available on [PayCollect — GPI](/api-reference/payments-v2/paycollect/gpi) sale flows).
...
- Set **`captureTxn`** to **`false`** in **Request body**, with `paymentData` (amount, currency, `billingData`) as for [PayCollect — GPI](/api-reference/payments-v2/paycollect/gpi).
