...
Card data (or network tokens) travel in the API request body. PayGlocal handles 3DS authentication, intelligent routing, and settlement on the back end. There is **no PayGlocal-hosted card form** in this mode.

<Note>
  **Not PCI DSS certified?** Use [PayCollect](/api-reference/payments-v2/paycollect/overview) instead — card details are collected on a PayGlocal-hosted page, and you do not need to handle card information in your application.
</Note>

### Key characteristics of PayDirect
...
PayDirect does **not** support UPI, net banking, or global alternative payment methods. Those are available on [PayCollect](/api-reference/payments-v2/paycollect/overview).
...
→ [PayDirect — GPI](/api-reference/payments-v2/paydirect/gpi) (complete request payload)
...
→ [PayDirect — Authorize Payment (Auth Only)](/api-reference/payments-v2/paydirect/auth-capture-authorise)
