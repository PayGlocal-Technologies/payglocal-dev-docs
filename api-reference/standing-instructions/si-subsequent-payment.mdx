...
```json Request (VARIABLE mandate)
{
  "merchantUniqueId": "TXN-20240601-001",
  "mandateId": "MND-<MANDATE_ID>",
  "paymentData": {
    "totalAmount": 500.00,
    "currencyCode": "INR"
  }
...
| Scenario | HTTP Code |
|---|---|
| `mandateId` not found | 400 |
| Mandate is inactive (paused or cancelled) | 400 |
| Mandate is exhausted (max debit count reached or expiry passed) | 400 |
| `paymentData.totalAmount` exceeds `maxAmount` on a VARIABLE mandate | 400 |
| `paymentData.totalAmount` supplied for a FIXED mandate | 400 |
| Merchant not authorised to use SI sale | 403 |
