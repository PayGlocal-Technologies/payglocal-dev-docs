...
description: "Authentication for the Merchant Onboarding APIs. Every request is signed with HmacSHA256 and sent via x-api-key + x-gl-digest headers."
---

<Note>
  This page covers authentication for the **Merchant Onboarding** APIs (`/gcc/v2/partner/merchant/*`).
...
| Header | Description |
|--------|-------------|
| `x-api-key` | Your static API Key, downloaded from the partner dashboard |
| `x-gl-digest` | A per-request HMAC-SHA256 signature, Base64-encoded |

Both headers are **required on every request**. Requests missing either header will be rejected.

---

## Credentials

Log in to the PayGlocal partner dashboard and navigate to **Settings → API Keys** to download your:

- **API Key** — a static, non-secret identifier. Safe to store in environment variables.
- **API Secret** — the signing key used to generate the digest. Treat this like a password. Never expose it in client-side code, logs, or version control.
...
|-------------|---------------|
| `GET` | The **full request URL** (including path and query string) |
...
URL="https://api.onboard.uat.payglocal.in/gcc/v2/partner/merchant/onboard/pg_onboard_abc123/status"
DIGEST=$(echo -n "$URL" | openssl dgst -sha256 -hmac "$API_SECRET" -binary | base64)
...
def get_headers(url: str) -> dict:
    digest = base64.b64encode(
        hmac.new(API_SECRET.encode(), url.encode(), hashlib.sha256).digest()
    ).decode()
    return {"x-api-key": API_KEY, "x-gl-digest": digest}
...
public class PayGlocalGet {
    ...
    public static void main(String[] args) throws Exception {
        String url    = "https://api.onboard.uat.payglocal.in/gcc/v2/partner/merchant/onboard/pg_onboard_abc123/status";
...
const url    = "https://api.onboard.uat.payglocal.in/gcc/v2/partner/merchant/onboard/pg_onboard_abc123/status";
const digest = crypto.createHmac("sha256", API_SECRET).update(url).digest("base64");
...
const options = {
  ...
  method:   "GET",
  headers: { "x-api-key": API_KEY, "x-gl-digest": digest },
};
...
| Using the body for GET request digest instead of the full URL | `401 Unauthorized` |
...
- Keys can be rotated from the partner dashboard at any time.
...
<CardGroup cols={2}>
  <Card title="Key Management" icon="key" href="/key-management/overview">
    Set up RSA-signed JWS tokens for the Payment APIs.
  </Card>
