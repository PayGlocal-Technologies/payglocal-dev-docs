...
<CardGroup cols={2}>
  <Card title="Checkout Flow (PayCollect)" icon="window" color="#1A6FE8" href="/payments/paycollect/overview">
    **PayGlocal collects card details** on a secure hosted page. You send a request and never touch card data.
    ...
    [Checkout Flow (PayCollect) details →](/payments/paycollect/overview)
  </Card>
  <Card title="Seamless Flow (PayDirect)" icon="code" color="#8B5CF6" href="/payments/paydirect/overview">
    **You collect card details** in your own checkout UI and send them to PayGlocal. No redirect.

    → Requires PCI DSS certification.

    [Seamless Flow (PayDirect) details →](/payments/paydirect/overview)
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Regular Payment" icon="credit-card" href="/payments/types/regular-payment">
    One-time, immediate payments — status tracking and refunds.
  </Card>
  <Card title="Recurring Payment" icon="rotate" href="/payments/types/recurring-payment">
    Standing Instructions — Auto Debit and On-Demand mandate flows.
  </Card>
  <Card title="Auth & Capture" icon="shield-check" href="/payments/types/auth-capture">
...
<Note>
  New here? Start with [Choose Your Integration](/payments/overview) for a high-level comparison of No-Code vs API, then come back to pick a mode and payment types.
</Note>
