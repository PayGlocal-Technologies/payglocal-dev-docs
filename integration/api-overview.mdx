...
description: "Understand PayGlocal's two API integration modes — PayCollect and PayDirect — and the payment types that work with both."
icon: "code"
---
...
<CardGroup cols={2}>
  <Card title="1. Integration mode" icon="shuffle" color="#1A6FE8">
    **How you collect card details** — on PayGlocal's hosted page (**PayCollect**) or in your own UI (**PayDirect**).
  </Card>
...
<CardGroup cols={2}>
  <Card title="PayCollect" icon="window" color="#1A6FE8" href="/merchant/paycollect/overview">
    **PayGlocal collects card details** on a secure hosted page. You send a request and never touch card data.
    ...
    [PayCollect details →](/merchant/paycollect/overview)
  </Card>
  <Card title="PayDirect" icon="code" color="#8B5CF6" href="/merchant/paydirect/overview">
    **You collect card details** in your own checkout UI and send them to PayGlocal. No redirect.
    ...
    [PayDirect details →](/merchant/paydirect/overview)
  </Card>
</CardGroup>
...
Payment types are **mode-agnostic** — each works with PayCollect or PayDirect.
...
| Payment type | PayCollect | PayDirect |
|---|:---:|:---:|
| Regular Payment | ✓ | ✓ |
| Recurring (SI) | ✓ | ✓ |
| Auth & Capture | ✓ | ✓ |
