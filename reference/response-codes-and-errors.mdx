...
| `401` | Unauthorized | Authentication failed. The `x-api-key` or `x-gl-digest` header is missing, invalid, or the digest does not match. |
...
| Invalid or missing `x-api-key` | `401` |
...
1. The `x-api-key` header must match your API Key exactly.
