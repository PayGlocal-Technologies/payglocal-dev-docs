...
<Steps>
  ...
  <Step title="Handle Callback — Listen for completion">
    ...
    **Option A — postMessage (recommended for real-time UX)**
    ...
    During intermediate steps (e.g. DigiLocker redirect), the iFrame may also post structured events such as `PARTNER_MERCHANT_VERIFICATION_REDIRECT`. See [Partner Onboarding Events](/set-up/partner-onboarding/events) for the full event reference.
